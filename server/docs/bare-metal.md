...
- Ổ đĩa: ~25GB cho hai venv, ~5GB cho model weight
...
```bash
cp .env.example .env
$EDITOR .env          # điền API_KEY và OPENAI_API_KEY
```
...
```json
{"status":"ok","models_loaded":["whisper","voxcpm","latentsync"],"gpu":"NVIDIA ..."}
```
...
|---|---|
| `ModuleNotFoundError: faster_whisper` lúc khởi động | Thiếu `requirements-models.txt`, hoặc chạy nhầm `python3` hệ thống thay vì `/opt/venv-main/bin/python` |
